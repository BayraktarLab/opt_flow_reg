...
def farneback(mov_img: Image, ref_img: Image) -> np.ndarray:
    flow = cv.calcOpticalFlowFarneback(mov_img, ref_img,
                                       None, pyr_scale=0.5, levels=3,
                                       winsize=51,
                                       iterations=3, poly_n=1, poly_sigma=1.7,
...
def denselk(mov_img: Image, ref_img: Image) -> np.ndarray:
    ...
    flow = cv.optflow.calcOpticalFlowSparseToDense(mov_img, ref_img,
                                                   None, grid_step=2, k=500)
    return flow
...
def rlof(mov_img: Image, ref_img: Image) -> np.ndarray:
    ...
    flow = cv.optflow.calcOpticalFlowDenseRLOF(mov_img, ref_img,
                                               None, rlofParam=param,
                                               interp_type=cv.optflow.INTERP_GEO)
    return flow
